        ...
        deepClone(
            allNavPages?.filter((item) =>
                item.tags?.some(
                    (t) =>
                        t ===
                        siteConfig("GAME_RECOMMEND_TAG", "Recommend", CONFIG)
                )
            )
        )
    );
        ...
        >
            <div
                ...
                className={`${siteConfig(
                    "FONT_STYLE"
                )} w-full h-full min-h-screen justify-center dark:bg-black dark:bg-opacity-50 dark:text-gray-300 scroll-smooth`}
            >
                <Style />
...
const LayoutSlug = (props) => {
    ...
    useEffect(() => {
        ...
        const existedIndex = recentGames.findIndex(
